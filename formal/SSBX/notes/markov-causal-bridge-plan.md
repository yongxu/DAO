...
- [x] 证明该对齐不是 direct language addition：复用 `no_direct_unification_by_addition`
...
- [x] 更新 `QuantumRelativityIntegration` 或新模块摘要，不破坏既有 no-go 边界

目的：不是推翻上一层 no-go，而是给出越过 no-go 的最小中介构造。
...
- 成功：`lake build SSBX` 通过，输出 `Build completed successfully (2885 jobs).`
- 备注：`lake build SSBX` 输出了既有 Wen 模块的 unused simp argument linter 警告；本轮新增 `QuantumRelativityMarkovBridge.lean` 无警告。
