...
| 项 | 内容 |
|---|---|
| Lean 出口 | `finite_causal_locality_bridge_summary` |
...
```text
FiniteCausalLocalFutureCandidate；
...
concreteCausalLocalFuture；
operatorCellGridCausalLocalFuture；
finite_causal_locality_bridge_summary。
```
...
| theorem | 文件 | 读法 |
|---|---|---|
| `FiniteCausalLocalFutureCandidate` | `QuantumRelativityFiniteCausalLocalityBridge.lean` | 每个 state 有 finite localFuture list |
| `local_future_iff_step` | `QuantumRelativityFiniteCausalLocalityBridge.lean` | localFuture membership 精确等于 one-step support |
| `local_future_implies_causal_before`、`nonlocal_not_one_step` | `QuantumRelativityFiniteCausalLocalityBridge.lean` | local future 可读成 causalBefore；nonlocal target 不是一步 successor |
...
目标：把 S29 的 displayed carrier 推成 constructor-recursive carrier。S30 不证明 global path enumeration、arbitrary-length causal intervals、full causal set local finiteness、general path integral、Lorentzian geometry、metric recovery 或 empirical closure；它只证明 finite `KernelPath` 的 carrier 可由 `refl` / `snoc` 递归生成，且 append 后 carrier 可读回为左 carrier 加右 path targets。

| 项 | 内容 |
|---|---|
| Lean 出口 | `kernel_path_recursive_carrier_bridge_summary` |
| 最低 theorem 形态 | 合取 `RecursiveKernelPathCarrierClosed`、`KernelPathAppendCarrierClosed`、`FiniteRecursiveKernelCarrierFamilyClosed`、S29 carrier boundary、concrete/twoRoute recursive carriers 与 Wen coverage |
| 失败记录 | 若构建失败，记录是否为 indexed recursive definition、append target algebra、carrier endpoint membership、two-route family simplification 或 overclaim mismatch |
| 文档更新 | 已新增《递归核路径载体候选 · Markov桥S30》 |
| 后续结构 | endpoint-indexed recursive carrier families、toy all-path carrier completeness、finite all-path support boundary、path integral、Lorentzian geometry 与 empirical closure |
...
kernelPathTargets；
kernelPathPoints；
kernelPath_start_mem；
kernelPath_stop_mem；
kernelPathTargets_append；
kernelPathPoints_append；
recursiveKernelPathCarrier；
recursive_kernel_path_carrier_closed；
twoRouteRecursiveKernelCarrierFamily；
finite_recursive_kernel_carrier_family_closed；
kernel_path_recursive_carrier_bridge_summary。
```
...
| theorem | 文件 | 读法 |
|---|---|---|
| `kernelPathTargets`、`kernelPathPoints` | `QuantumRelativityKernelPathRecursiveCarrierBridge.lean` | finite kernel path 的 carrier 由 constructors 递归生成 |
| `kernelPath_start_mem`、`kernelPath_stop_mem` | `QuantumRelativityKernelPathRecursiveCarrierBridge.lean` | 起点与终点都在递归 carrier 中 |
| `kernelPathTargets_append`、`kernelPathPoints_append` | `QuantumRelativityKernelPathRecursiveCarrierBridge.lean` | append 后 carrier 可读回为左 carrier 加右 targets |
| `recursiveKernelPathCarrier` | `QuantumRelativityKernelPathRecursiveCarrierBridge.lean` | 任意 finite kernel path 自动得到 S29 displayed carrier |
| `twoRouteRecursiveKernelCarrierFamily` | `QuantumRelativityKernelPathRecursiveCarrierBridge.lean` | two-route upper/lower 形成 finite recursive carrier family |
| `kernel_path_recursive_carrier_bridge_summary` | `QuantumRelativityKernelPathRecursiveCarrierBridge.lean` | S30 recursive kernel path carrier boundary 已关闭 |

## S31 · 端点索引递归载体族候选

目标：把 S30 的 recursive carriers 包成 endpoint-indexed finite carrier family。S31 不证明 all-path completeness、global path enumeration、arbitrary-length causal intervals、general path integral、Lorentzian geometry、metric recovery 或 empirical closure；它只证明 family member 共享端点读法、two-route carrier/weight lists 与 displayed weight sum。

| 项 | 内容 |
|---|---|
| Lean 出口 | `endpoint_indexed_recursive_carrier_bridge_summary` |
| 最低 theorem 形态 | 合取 `EndpointIndexedRecursiveCarrierFamilyClosed`、`TwoRouteEndpointIndexedRecursiveFamilyClosed`、S30 recursive carrier boundary、twoRoute carrier/weight list 与 Wen coverage |
| 失败记录 | 若构建失败，记录是否为 dependent list member、carrier list simplification、weight sum simplification 或 overclaim mismatch |
| 文档更新 | 已新增《端点索引递归载体族候选 · Markov桥S31》 |
| 后续结构 | two-route toy carrier-family completeness、finite all-path support boundary、general all-path enumeration、path integral 与 empirical closure |

通过判准：

```text
EndpointIndexedRecursiveKernelCarrierFamily；
EndpointIndexedRecursiveKernelCarrierFamily.member_start_mem；
EndpointIndexedRecursiveKernelCarrierFamily.member_stop_mem；
EndpointIndexedRecursiveKernelCarrierFamily.member_causal_before；
EndpointIndexedRecursiveKernelCarrierFamily.member_weight_readback；
...
| theorem | 文件 | 读法 |
|---|---|---|
| `EndpointIndexedRecursiveKernelCarrierFamily` | `QuantumRelativityEndpointIndexedRecursiveCarrierBridge.lean` | recursive carriers 可包成同端点 finite family |
| `member_start_mem`、`member_stop_mem`、`member_causal_before`、`member_weight_readback` | `QuantumRelativityEndpointIndexedRecursiveCarrierBridge.lean` | 任意 member 可读回 shared endpoints、causalBefore 与 path weight |
| `twoRouteEndpointIndexedRecursiveCarrierFamily` | `QuantumRelativityEndpointIndexedRecursiveCarrierBridge.lean` | two-route upper/lower carriers 被端点索引 |
| `two_route_endpoint_indexed_recursive_family_weight_sum` | `QuantumRelativityEndpointIndexedRecursiveCarrierBridge.lean` | two-route displayed carrier weights 总和为 `2` |
| `endpoint_indexed_recursive_carrier_bridge_summary` | `QuantumRelativityEndpointIndexedRecursiveCarrierBridge.lean` | S31 endpoint-indexed recursive carrier family boundary 已关闭 |

## S32 · 双路径递归载体族完备候选

目标：只在 two-route displayed family 上证明 completeness：family 恰好由 upper/lower displayed recursive carriers 组成，任意 displayed member 是 upper 或 lower，且权重为 `1`。S32 不证明 arbitrary endpoint completeness、all KernelPath enumeration、global path enumeration、general path integral、Lorentzian geometry、metric recovery 或 empirical closure。

| 项 | 内容 |
|---|---|
| Lean 出口 | `two_route_displayed_carrier_completeness_bridge_summary` |
...
two_route_displayed_recursive_family_member_carrier_cases；
...
| theorem | 文件 | 读法 |
|---|---|---|
| `twoRouteDisplayedRecursiveCarrierFamily` | `QuantumRelativityTwoRouteDisplayedCarrierCompletenessBridge.lean` | two-route displayed recursive carrier family |
