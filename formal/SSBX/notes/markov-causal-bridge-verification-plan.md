...
| 真实干涉律 | S5/S5b/S5c/S5d/S5e/S5f/S5g/S5h 已有 path amplitude、非零 witness、two-path finite cancellation candidate、discrete phase-label candidate、edge-action phase accumulation candidate、finite path-family sum candidate、finite path-sum algebra candidate 与 endpoint-indexed finite family candidate；还需要 all-path enumeration、一般 path integral、连续相位动力学、观测 ledger 或可测相消 theorem |
...
| V6h | 引入 endpoint-indexed finite path-family candidate | 已由 `endpoint_indexed_path_family_bridge_summary` 关闭 endpoint-indexed conversion、sum preservation 与 two-route endpoint-indexed cancellation；filter/support normalization、all-path enumeration、continuous action law 与 path integral 仍 pending |
