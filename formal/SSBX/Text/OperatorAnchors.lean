...
/-- All 13 BaguaWen primary instruction tokens (v2), with text-catalogue status. -/
def l0OperatorAnchors : List L0OperatorAnchor :=
  ...
  -- v2 (2026-05-17)
  , { kind := .branchYaoYang, catalogueIds := [],             missingForms := ["比阳"] }
  ]
...
theorem reservedTokenAnchors_length :
    reservedTokenAnchors.length = 24 := by
  native_decide
