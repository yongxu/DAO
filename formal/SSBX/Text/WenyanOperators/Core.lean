...
inductive OperatorId where
  ...
  | N_7 | N_8 | I_1 | I_2 | I_3 | I_4 | I_5 | I_6
    ...
    /-- 句末虚词 乎: interrogative final.  Bool identity marker; modal axis = M_5 可 (interrogative possibility). -/
    | S_21
    /-- 句末虚词 矣: perfective final.  Bool identity marker; modal axis = perfective aspect (与 S_18 已 关联). -/
    | S_22
    /-- 句末虚词 哉: exclamatory final.  Bool identity marker; modal axis = M_1 必 (exclamatory necessity). -/
    | S_23
    /-- 句末虚词 兮: lyrical final.  Bool identity marker; modal axis = lyrical/aesthetic marker. -/
    | S_24
    | H_1 | H_2
