...
namespace SSBX.Foundation.JianModeKernel

open SSBX.Foundation.Yi
open SSBX.Foundation.Kernel
...
/-! ## § 2 JianMode ↔ KernelRole bijection
  ...
  `kernelRole` defined inside `SSBX.Foundation.Yi.JianMode` for dot notation. -/

end SSBX.Foundation.JianModeKernel

namespace SSBX.Foundation.Yi.JianMode

open SSBX.Foundation.JianModeKernel (KernelRole)

/-- Each ontological mode's dynamical role. -/
def kernelRole : SSBX.Foundation.Yi.JianMode → KernelRole
  | .sheng => .continuation
  | .shou  => .reception
  | .yuan  => .origin
  | .shen  => .penetration
...
end SSBX.Foundation.Yi.JianMode
