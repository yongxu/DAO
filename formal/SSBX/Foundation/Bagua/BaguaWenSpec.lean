/-
# BaguaWenSpec — L0 内核：baguaWen 之冻结 23-token

Frozen at HEAD `1cd30f8`, corresponding to `notes/baguaWen-spec.md`.

This module is the **L0 internal kernel** — the 23 baguaWen primitives
...
Defines:
  ...
  · `primaryTokens`   — the 13 verb tokens (de-duplicated, ordered)
  · `shiTokens`       — 3 time-mode tokens
  · `yaoTokens`       — 6 line-position tokens
  · `reservedTokens`  — full L0 reserved set (23 tokens, excluding numerals)

Lemmas verified by `native_decide`:
  · `primaryTokens` length = 13, no duplicates
  · `reservedTokens` length = 23
...
以避命名空间冲突）。L0 之 23 token 与 L1 之扩展 def 名严格互斥
