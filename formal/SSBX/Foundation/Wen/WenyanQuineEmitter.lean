...
def emitCellFrom (cur target : R8) : List YiInstr :=
...
theorem emitCellFrom_length_le (cur target : R8) :
...
def emitCellInit (cur target : R8) (oldHistory : List R8) : YiState :=
  { cur := cur
  , history := oldHistory
  , pc := 0
...
theorem emitCellFrom_correct_zero_flip :
    let cur : R8 := (Hexagram.heaven, Shi.jin)
    let target : R8 := (Hexagram.heaven, Shi.ji)
...
theorem emitCellFrom_correct_six_flips :
    let cur : R8 := (Hexagram.heaven, Shi.jin)
    let target : R8 := (Hexagram.earth, Shi.wei)
    ((emitCellInit cur target []).runFuel (emitCellFrom cur target).length).cur = target
      ∧ ((emitCellInit cur target []).runFuel (emitCellFrom cur target).length).history = [target] := by
  native_decide
