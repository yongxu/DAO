/-
# Wen.Xiang.Layers — five interface layer aliases

The five interface tower layers of `wen-algebra` v0.4 §3.1, exposed as
...
names.  Per v0.4 §3.1, layers `n ≥ 5` are **IR-internal only** and
...
/-- **时卦 / Temporal Hexagram** — `X 4`.  256 elements, four image
    coordinates: per v0.4 §4.4 the first three are the `Hexagram` factor
...
Per `wen-algebra` v0.4 §4.3, |X n| = 4ⁿ.  Proofs go via Mathlib's
...
/-- 太极 — the unique element of `Origin = X 0`.  This is the empty
    tuple of `Image` atoms; in v0.4 §0.2 it is the trivial 道. -/
