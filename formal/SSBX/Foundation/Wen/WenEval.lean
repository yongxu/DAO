...
顶层入口 `«解执»`：String → Hexagram → Nat → Option (List R8)
...
`«解执» «道判源» heaven 10` 之 head = `some (heaven, Shi.ji)` —— 文之源经 String 入，
...
def wenEval (n : Nat) (s : YiState) : List R8 :=
...
def «解执» (src : String) (h : Hexagram) (n : Nat) : Option (List R8) :=
...
theorem «端到端_乾» :
    («解执» «道判源» Hexagram.heaven 10).bind (fun cells => cells.head?)
      = some (Hexagram.heaven, Shi.ji) := by native_decide

/-- 端到端 2：坤卦输入 → 解执 → 编码列首 = (坤, 已). -/
theorem «端到端_坤» :
    («解执» «道判源» Hexagram.earth 10).bind (fun cells => cells.head?)
...
/-- 端到端 4：非法源 → 解执 = none. -/
theorem «端到端_非法» :
    «解执» "«未知»" Hexagram.heaven 10 = none := by native_decide
