/-
# Wen.SquaringTower.Dot — F₂-bilinear dot product on `X n`

`wen-algebra` v0.2 §5: the canonical pairing on the 𝕏ⁿ tower.  Per §5.2:
...
* `X.dot` — recursive definition mirroring the squaring tower.
...
* `dot_basisAlpha / dot_basisBeta` — basis pairings recover coordinates
  (§5.4 Sense 1).
* `coord_independent_recovery` — the full v0.2 §5.4 Sense 1 + Sense 2
...
* `nondegenerate` / `nondegenerate_iff` / `nondegenerate_iff_coord` —
  v0.2 §5.3.3 non-degeneracy in three forms (forward implication, iff
...
/-! ## § 4 Canonical basis (`wen-algebra` v0.2 §4.2)
...
image factors (no cross-talk).  This is the v0.2 hallmark of typed
self-duality on the 𝕏ⁿ tower (§5.4). -/

/-- Coordinate-independent recovery of `U i` from its two basis
    pairings.  Per `wen-algebra` v0.2 §5.4, this expresses both Sense 1
...
recovery theorems.  The iff form `nondegenerate_iff` matches the v0.2
...
/-- Non-degeneracy as an iff, matching `wen-algebra` v0.2 §6.1's specification:
...
/-- Per-coordinate restatement of non-degeneracy (v0.2 §6.1 form): the origin
