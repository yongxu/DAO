...
theorem wenSurface_operator_catalogue_counts :
    fullOperatorSignatures.length = 371
      ...
      ∧ (domainGapDetailKindOperatorIds .applicationMechanic).length = 10
      ∧ (domainGapDetailKindOperatorIds .identityNoop).length = 18
      ∧ (domainGapDetailKindOperatorIds .projectionModel).length = 102
      ∧ (domainGapDetailKindOperatorIds .pairCarrierLaw).length = 57
      ∧ (domainGapDetailKindOperatorIds .facetCarrierLaw).length = 12
      ∧ (domainGapDetailKindOperatorIds .singletonAggregateLaw).length = 15
