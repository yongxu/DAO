...
theorem wenSurface_operator_catalogue_counts :
    fullOperatorSignatures.length = 371
      ...
      ∧ exactStructuralHelperStrongOperatorIds.length = 208
      ...
      ∧ (domainGapKindOperatorIds .carrierConstructorOnly).length = 63
      ∧ (domainGapKindOperatorIds .predicateAnchorOnly).length = 9
      ∧ (domainGapKindOperatorIds .truthMarkerOnly).length = 4
      ∧ (domainGapKindOperatorIds .catalogueShapeOnly).length = 41
