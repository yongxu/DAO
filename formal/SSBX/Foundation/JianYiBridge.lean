...
/-! ## § 1 Dimension → DaoLevel
  ...
  Defined inside `SSBX.Foundation.Jian.Dimension` to enable dot notation. -/

namespace SSBX.Foundation.Jian.Dimension

open SSBX.Foundation.Yi (DaoLevel)

/-- Each linguistic dimension's dominant ontological tier. -/
def daoLevel : SSBX.Foundation.Jian.Dimension → DaoLevel
...
end SSBX.Foundation.Jian.Dimension

/-! ## § 2 Op → DaoLevel (via Dimension)

  Defined inside `SSBX.Foundation.Jian.Op` to enable dot notation. -/

namespace SSBX.Foundation.Jian.Op

open SSBX.Foundation.Yi (DaoLevel)

/-- Each operator's dominant ontological tier (via its dimension). -/
def daoLevel (o : SSBX.Foundation.Jian.Op) : DaoLevel := o.dim.daoLevel

end SSBX.Foundation.Jian.Op

/-! ## § 3 Verification + Wen-level reading -/

namespace SSBX.Foundation.JianYiBridge
...
end SSBX.Foundation.JianYiBridge
