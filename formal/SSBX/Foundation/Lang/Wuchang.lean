...
| --- | --- | --- | --- |
| 仁 | 木 | `(qianqian.flipY1, false)` = `xoooooo` | flip 初爻 (initiate) |
| 义 | 金 | `(qianqian.flipY2, false)` = `oxooooo` | flip 二爻 (discriminate) |
| 礼 | 火 | `(qianqian.flipY3, false)` = `ooxoooo` | flip 三爻 (manifest) |
| 智 | 水 | `(qianqian.flipY4, false)` = `oooxooo` | flip 四爻 (perceive) |
...
def benevolence : Cell128 :=
  (Hexagram.mk .yin .yang .yang .yang .yang .yang, false)

/-- 义 (righteousness, 金): flip 二爻. -/
def righteousness : Cell128 :=
  (Hexagram.mk .yang .yin .yang .yang .yang .yang, false)

/-- 礼 (propriety, 火): flip 三爻. -/
def propriety : Cell128 :=
  (Hexagram.mk .yang .yang .yin .yang .yang .yang, false)

/-- 智 (wisdom, 水): flip 四爻. -/
def wisdom : Cell128 :=
  (Hexagram.mk .yang .yang .yang .yin .yang .yang, false)

/-- 信 (integrity, 土): flip y1..y4 simultaneously = 仁⊕义⊕礼⊕智. -/
def integrity : Cell128 :=
  (Hexagram.mk .yin .yin .yin .yin .yang .yang, false)
...
/-- 仁 = 震䷲ (☳☳ = pure thunder, 木). -/
def benevolence : Cell128 :=
  (Hexagram.mk .yang .yin .yin .yang .yin .yin, false)

/-- 义 = 兑䷹ (☱☱ = pure lake, 金). -/
def righteousness : Cell128 :=
  (Hexagram.mk .yang .yang .yin .yang .yang .yin, false)

/-- 礼 = 离䷝ (☲☲ = pure fire, 火). -/
def propriety : Cell128 :=
  (Hexagram.mk .yang .yin .yang .yang .yin .yang, false)

/-- 智 = 坎䷜ (☵☵ = pure water, 水). -/
def wisdom : Cell128 :=
  (Hexagram.mk .yin .yang .yin .yin .yang .yin, false)

/-- 信 = 坤䷁ (☷☷ = pure earth, 土). -/
def integrity : Cell128 :=
  (Hexagram.mk .yin .yin .yin .yin .yin .yin, false)
