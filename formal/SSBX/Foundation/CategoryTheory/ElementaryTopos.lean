...
2. cartesian closed (`CartesianMonoidalCategory` with `MonoidalClosed`), and
...
* `CategoryTheory.MonoidalClosed` over a `CategoryTheory.CartesianMonoidalCategory`
  (in `Mathlib.CategoryTheory.Monoidal.Closed.Basic` and
   ...
   note the old `CartesianClosed` is now a deprecated alias for `MonoidalClosed`,
   see the deprecation block in `Mathlib.CategoryTheory.Monoidal.Closed.Cartesian`),
...
* `ElementaryTopos C` -- a `Prop`-valued mixin asserting that `C` carries
  finite limits, a subobject classifier, and (via instance parameters) the
  cartesian-closed structure.
* `ElementaryTopos.Ω` -- the chosen subobject classifier object (re-export).
* `ElementaryTopos.PowerObject A` -- the *power object* `Ω ^^ A`,
  i.e. the internal hom from `A` into `Ω`.

## Main results / instances

* `ElementaryTopos.toHasTerminal`, `toHasFiniteProducts`, `toHasPullbacks`
  -- derived from `HasFiniteLimits`.
* `Types.subobjectClassifier`, `Types.hasSubobjectClassifier`
  -- the classical Lawvere construction `Ω := ULift Prop`, with
  `truth : PUnit ⟶ ULift Prop` picking out `True`.
* `instance : ElementaryTopos (Type u)`
  -- the prototypical topos.

## References

* [Lawvere, F. W. and Tierney, M., *Quantifiers and sheaves*][LT70]
  (Actes du Congres International des Mathematiciens, 1970, vol. 1, pp. 329-334;
  origin of the *elementary* topos).
* [Mac Lane, S. and Moerdijk, I., *Sheaves in Geometry and Logic: A First
  Introduction to Topos Theory*][MM92] (Springer, 1992; standard reference,
  esp. Chap. I-IV for definitions and Chap. IV.5 for the `Type` case).
* [Johnstone, P. T., *Sketches of an Elephant: A Topos Theory Compendium*][Joh02]
  (Oxford, 2002).
* nLab: <https://ncatlab.org/nlab/show/topos>
  and <https://ncatlab.org/nlab/show/elementary+topos>.

## Implementation notes

`ElementaryTopos` is declared as a `Prop` mixin whose `extends` clauses are the
two propositional component classes (`HasFiniteLimits`, `HasSubobjectClassifier`).
The data-bearing classes `CartesianMonoidalCategory` and `MonoidalClosed` are
taken as *instance parameters* rather than `extends`, since their content is data
(a choice of binary products / internal homs) rather than a proposition. This is
the same pattern used by, e.g.,
`CategoryTheory.ObjectProperty.IsMonoidalClosed` in
`Mathlib.CategoryTheory.Monoidal.Subcategory`, which similarly assumes
`[MonoidalClosed C]` as an instance parameter rather than extending it.

This design also matches the way `CategoryTheory.Regular` in
`Mathlib.CategoryTheory.RegularCategory.Basic` extends `HasFiniteLimits` only,
keeping data-bearing structures as parameters.

The canonical instance on `Type u` uses `Ω := ULift.{u} Prop`, lifting the
classical `Prop` (which lives in `Type 0`) into the ambient universe. The
characteristic map of a mono `m : U ⟶ X` sends `x : X` to the proposition
"there exists `u : U` with `m u = x`" (which, since `m` is injective, is
equivalent to `x ∈ Set.range m`).
-/
...
/--
An **elementary topos** in the sense of Lawvere--Tierney is a category equipped
with finite limits, a cartesian-closed structure, and a subobject classifier.

We model this as a `Prop`-valued class extending `HasFiniteLimits` and
`HasSubobjectClassifier`, and requiring `CartesianMonoidalCategory C` and
`MonoidalClosed C` as instance parameters (these carry data rather than a
proposition).

Equivalently (cf. Mac Lane--Moerdijk *Sheaves in Geometry and Logic*, I.1):
*A category `C` is an elementary topos iff it has finite limits, is cartesian
closed, and has a subobject classifier*. -/
...
This is a re-export of `CategoryTheory.HasSubobjectClassifier.Ω`. We restate it
here so that `ElementaryTopos.Ω C` is the canonical entry-point. -/
...
/-- The **power object** of `A` in an elementary topos is the internal hom
`Ω ^^ A`. In the topos of sets it is the powerset `2^A`.

This uses the standard scoped notation `_ ^^ _` from
`Mathlib.CategoryTheory.Monoidal.Closed.Cartesian`, which expands to
`(ihom A).obj _`.

The category `C` is left implicit and inferred from the argument `A`. -/
...
We exhibit the classical Lawvere construction: in `Type u`, set
`Ω := ULift.{u} Prop`. The truth value `⊤_ ⟶ Ω` picks out `True`, and the
characteristic map of a mono `m : U ⟶ X` sends `x : X` to `∃ u, m u = x`. -/

namespace Types

open Subobject Limits

/-- The truth morphism in `Type u`: a function `PUnit ⟶ ULift.{u} Prop`
sending `PUnit.unit` to `ULift.up True`. -/
noncomputable def truth : PUnit.{u + 1} ⟶ ULift.{u} Prop :=
  ↾fun _ => ULift.up True

/-- The characteristic map of a mono `m : U ⟶ X` in `Type u`:
sends `x : X` to `ULift.up (∃ u, m u = x)`.

By monicity (i.e. injectivity in `Type`), this is `True` iff `x` lies in the
image of `m`. -/
noncomputable def χ {U X : Type u} (m : U ⟶ X) [Mono m] : X ⟶ ULift.{u} Prop :=
  ↾fun x => ULift.up (∃ u, m u = x)

/-- The image of `x : X` under `χ m`, as a `Prop`. -/
@[simp] lemma χ_apply_down {U X : Type u} (m : U ⟶ X) [Mono m] (x : X) :
    (χ m x).down = (∃ u, m u = x) := rfl

/-- The image of `p : PUnit` under `truth`, as a `Prop`. -/
@[simp] lemma truth_apply_down (p : PUnit.{u + 1}) : (truth p).down = True := rfl

/-- The truth-square commutes: composing `m` with `χ m` yields the constant
"True" map. -/
lemma comp_χ_eq {U X : Type u} (m : U ⟶ X) [Mono m] :
    m ≫ χ m = Types.isTerminalPUnit.from U ≫ truth := by
  ext u
  -- Goal is an `↔` on the `.down` of the two sides.
  -- LHS: `(∃ u', m u' = m u)`, RHS: `True`.
...
lemma isPullback_χ_truth {U X : Type u} (m : U ⟶ X) [Mono m] :
  ...
  · -- injectivity of the pair (m, terminal): m is mono so injective
    intro u₁ u₂ ⟨h, _⟩
    exact (CategoryTheory.mono_iff_injective m).mp inferInstance h
  · -- existence: for `x : X` and `p : PUnit` with `χ m x = truth p`,
    -- the down-equality says `∃ u, m u = x`, so we get a witness
    intro x p hxp
...
lemma χ_unique {U X : Type u} (m : U ⟶ X) [Mono m]
  ...
  constructor
  · -- if χ' x is True, use existence to get u with m u = x
    intro hx
    have hχ'eq : χ' x = ULift.up True := by
      apply ULift.ext
      exact propext ⟨fun _ => trivial, fun _ => hx⟩
    obtain ⟨u, hu, _⟩ := hex x PUnit.unit (by
      rw [hχ'eq]; rfl)
    exact ⟨u, hu⟩
  · -- if ∃ u, m u = x then χ' (m u) = True by commutativity
    rintro ⟨u, rfl⟩
    -- `hw : m ≫ χ' = (terminal map) ≫ truth` as morphisms in `Type u`.
    -- Apply `ConcreteCategory.congr_hom` to evaluate at `u`.
    have h := ConcreteCategory.congr_hom hw u
    -- h : χ' (m u) = truth (Types.isTerminalPUnit.from U u)
    -- Take `.down` and unfold `truth`.
    have h' := congrArg ULift.down h
    simpa using h'

/-- The subobject classifier on `Type u`: `Ω = ULift.{u} Prop`, with `truth`
picking out `True` and `χ m` sending `x` to `∃ u, m u = x`.

This is the standard Lawvere construction (Mac Lane--Moerdijk IV.1). The
universe lifting `ULift.{u} Prop` is needed because `Prop : Type 0` while we
need `Ω : Type u`. -/
...
/-- The category `Type u` has a subobject classifier (the classical Lawvere construction). -/
...
This is the prototypical Lawvere--Tierney example. The required four ingredients
are supplied by:

* `CartesianMonoidalCategory (Type u)` -- `Mathlib.CategoryTheory.Monoidal.Types.Basic`,
* `MonoidalClosed (Type u)` -- `Mathlib.CategoryTheory.Monoidal.Closed.Types` (l.46),
* `HasLimitsOfSize (Type u)` -- `Mathlib.CategoryTheory.Limits.Types.Limits` (l.185f),
  which yields `HasFiniteLimits` via `hasFiniteLimits_of_hasLimitsOfSize`,
