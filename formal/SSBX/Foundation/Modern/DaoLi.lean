...
| T₆ V₄ 超八面体扩张      | BaguaAlgebra § 11（Hexagram.cuo） / Yi (cuo/zong/cuoZong) | § 3 `V4_orbit_of_qian_distinct` |
...
形式来自 KleeneCarrier.halts_undecidable_internally 与 BaguaAlgebra.Sheng。

注：原 `GodelLi.halts_undecidable_internally` 已移至 `KleeneCarrier.lean`
（轴心拆分后），结论与原相同，从四件具名原子公理派生。 -/

/-- **道 ⊨ 理 之精确陈述**：Lean kernel 证明「Halts 在 YiInstr × Hexagram
    上不被任何 Lean Bool 函数判定」（即 KleeneCarrier.halts_undecidable_internally）。
...
/-- **V₄ 元素 2**：错（cuo）—— yao-wise 取反。 -/
def V4_cuo : Hexagram → Hexagram := Hexagram.cuo

/-- **V₄ 元素 3**：综（zong）—— yao-顺序反转。 -/
def V4_zong : Hexagram → Hexagram := Hexagram.zong
...
/-- **V₄ 在 taiHex 上之轨道大小 ≥ 2**：cuo 像 ≠ id 像（具体卦不等之见证）。
    （注：在 taiHex 上 V₄ 之 stabilizer 含 zong，故 |orbit| = 2，非 4。
