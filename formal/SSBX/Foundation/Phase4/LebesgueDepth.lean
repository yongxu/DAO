...
Companion: `四级生成_太极两翼四象八卦/统计 · 从元到测.md` § 7-12 (深化)

`Phase4/Lebesgue.lean` 已建 lintegral 基础（线性 / 单调 / 指示 / 连续 Bayes / Markov-Chebyshev /
...
Phase4/Lebesgue.lean 给"积分之代数性质"（线性 / 单调 / Markov / Chebyshev-anchor）；
...
namespace SSBX.Foundation.Phase4.LebesgueDepth
...
theorem wlln_tendsto_zero_variance {Ω : Type*} {m : MeasurableSpace Ω}
  ...
  have hzero : ∀ n, μ {ω | (ε : ℝ) ≤ |X n ω - μ[X n]|} = 0 := fun n =>
    SSBX.Foundation.Phase4.Lebesgue.wlln_zero_variance (hX n) hε (hvar n)
  simp only [hzero]
  exact tendsto_const_nhds
...
`Phase4/Lebesgue.lean` 已构 dayanMeasure（finite ↪ continuous 嵌入）。
...
open SSBX.Foundation.Phase4.Lebesgue
...
end SSBX.Foundation.Phase4.LebesgueDepth
