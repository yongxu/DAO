...
Companion: `四级生成_太极两翼四象八卦/统计 · 从元到测.md` § WLLN-IID 深化

`Phase4/LebesgueDepth.lean` 已构 WLLN tendsto-form scoped 版本——其条件是
...
namespace SSBX.Foundation.Phase4.IIDWlln
...
theorem wlln_iid_tendsto [IsProbabilityMeasure μ] {X : ℕ → Ω → ℝ}
  ...
  exact SSBX.Foundation.Phase4.LebesgueDepth.wlln_tendsto_chebyshev_scoped
...
end SSBX.Foundation.Phase4.IIDWlln
