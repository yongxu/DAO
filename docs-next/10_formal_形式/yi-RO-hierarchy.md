...
## 第十一部分：平方塔视角 — R₀..R₈ 之自相似子塔 (2026-05-11 增补)

第一部分之**incrementing 视角** (R_n → R_{n+1} 由 +1 bit) 是本文主线. 然而 R₀..R₈ 内之
下标 ∈ {0, 1, 2, 4, 8} 之 5 层有另一条**更紧之联系** — 它们由"平方"算子 S(A) := A × A
递推生成:

```
T₀ = R₀ = Unit                                  (1 元素 — 太极, 平方算子之不动点)
T₁ = R₁ = T₀ + T₀ = Bool                        (2 元素 — 由奠基算子 O₀ 给出)
T_{k+1} = S(T_k) = T_k × T_k     (k ≥ 1)         (平方递推)

代入: T₂ = R₂ = 4 (四象), T₃ = R₄ = 16 (面 Mian), T₄ = R₈ = 256 (Cell256)
```

塔层 = {R₀, R₁, R₂, R₄, R₈} 之下标按 2 的幂走, 与塔深 k 之关系 |T_k| = R_{2^(k-1)} for k≥1.

### 11.1 平方规则

> **R_{2n} = R_n × R_n for n ∈ {1, 2, 4}** (即在塔内层之自平方)

塔外层 R₃, R₅, R₆, R₇ 不在此规则内 — 它们由塔内层之**多种乘积路径**之交叉给出
(详 [义理/Z_平方塔自相似规则.md](../../义理/Z_平方塔自相似规则.md) § 7).

### 11.2 奠基步 R₀ + O₀ → R₁

S(R₀) = R₀ × R₀ = R₀, **平方算子在 R₀ 退化**. R₁ 需外加奠基算子 O₀:

| O₀ 表述 | 形式 | 直观 |
|---------|------|------|
| 协积 | R₁ := R₀ + R₀ | 复制 + 标签 |
| 动静 | O₀ = "动" | R₀ 在 O₀ 下分裂为 {静, 动} |
| substance/field | O₀ = 二选赋形 | R₁ = {实, 虚} |

三者同构, 语义不同, doctrine 同列 (2026-05-11 user-confirmed).

### 11.3 元/爻 配对原则

每一步 T_{k+1} = T_k × T_k 之两个 T_k 因子**非对称**:
- 一个承担**元** (carrier, 被分类的内容)
- 另一个承担**爻** (dimension, 分类所沿之轴)

具体分配是 doctrine 选择. 在 R₄ = R₂ × R₂ 处, **首选读法**:
- 实/虚 (内层) = **元** — 被分类之实虚态
- 本/征 (外层) = **爻** — zong-fixed / zong-mobile 之分类轴

**对偶读法**: 本/征 = 元, 实/虚 = 爻. 两者皆 doctrine 合法 (无群结构偏好).

### 11.4 R₈ 之两视角并存

R₈ Cell256 = (Z/2)⁸ 有两个等价主因式分解:

| 视角 | 表式 | 适用 |
|------|------|------|
| **incrementing** (本文主线) | R₈ = R₆ × V₄ = Hex × Shi | 时空 anchor / Born rule / 算子代数 |
| **squaring** (本节增补) | R₈ = R₄ × R₄ = Mian × Mian | fractal / 自相似 / 元爻配对 |

两者通过 (Z/2)⁸ 同一群之 bit 重排同构. 选择由语义需要决定. 详 [义理/Z_平方塔自相似规则.md](../../义理/Z_平方塔自相似规则.md).

### 11.5 道与 (1,1)-quarter 在平方塔下之归位

道 = R₈.origin = (T₃-元 = 0, T₃-爻 = 0) — 平方塔末层 T₄ 之零元.
(1,1)-quarter = V₄ 之 今-slice = 4 个 R₄ fiber = R₂ × R₄ = 64 元素子集 — 含 R₄ 自相似嵌套.
真ₛ (dao-slice) 同结构, 与 (1,1)-quarter 互为 V₄ 极对.
