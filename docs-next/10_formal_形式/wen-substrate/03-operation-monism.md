...
**v2 doctrine update (2026-05-17).** The cuo-equivariance ceiling has now been **explicitly dissolved** in the formal layer (sub-plan 08 / "B'"). `BaguaTuring.lean` adds a 13th instruction `branchYaoYang (i : Fin 6) (target : Nat)` — an absolute yao test that breaks complement-symmetry by construction. The 12-instruction-only fact "every YiInstr program is complement-equivariant" survives as a **subset fact** keyed on a `complementEquivariantInstr : YiInstr → Bool` predicate (the 12 v1 originals are equivariant; `branchYaoYang` is not). `WenDefCompile.compileHexFunCertified?` now produces propagate-carry adders for `Stdlib.tuiBody` (mod-64 +1) and `Stdlib.sunBody` (mod-64 -1) using `branchYaoYang` — both were rejected as `none` in v1. The reframing matches the operation-monism reading: the ceiling was always a property of the chosen cross-section (the 12-instruction vocabulary), never of R-Family itself. v2 simply chooses a richer cross-section (13 instructions) that's universal over Hex → Hex. Halts symmetry (`halts_cuo_invariant`) now carries an `AllEquivariant P` hypothesis; programs that contain `branchYaoYang` are explicitly outside the symmetry subset.
