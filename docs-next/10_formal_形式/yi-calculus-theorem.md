...
| # | 定理 | 内容 | Lean 状态 |
|---|---|---|---|
| A | R3 (本征 4+4) | zong involution 强制 4 substrate + 4 mark 划分 | ✓ BenZheng.lean |
| B | substrate = 定常波 | 4 本是 Σ³ 上仅有的 4 个 boundary-preserving 配置 | ✓ |
| C | mark = 4 phase quadrant | 4 征是 (sum, endpoint-difference) phase plane 的 4 representative | ✓ |
| D | cuo–zong V₄ 代数 | cuo = P (parity), zong = T (time-reversal), 错综 = PT | ✓ |
| E | substrate–mark = ∫–d 划分 | 离散 Newton-Leibniz duality | ✓ |
| F | R6 (64 卦 4 quadrant) | 本本/本征/征本/征征 各 16，cuo 保 quadrant，zong 跨换 mixed | ✓ BenZheng.lean |
| G | 与连续 1D 标量场对应 | 8 trigram = 3-sample sign-pattern 的完全枚举 | informal |
| **H** | **R7 = (Z/2)⁷ = 128 + 因 (yīn) axis** | **R7 = R6 × YinBit = Hexagram × {0,1}；引入「过去印记」第 7 个 bit** | refactor pending |
| **I** | **R8 = (Z/2)⁸ = 256 + 果 (guǒ) axis** | **R8 = R7 × GuoBit = Hexagram × Bool²；引入「未来投影」第 8 个 bit** | refactor pending |
| **J** | **Shi V₄ = (因, 果) emerges at R8** | **{道, 已, 今, 未} 不是 R7 单层 atom，是 R7⊗R8 双 axis tensor；道 = (0,0) = V₄ identity** | refactor pending |
| **K** | **R-hierarchy R1..R8 全 (Z/2)ⁿ closure** | **5 层均纯 (Z/2)ⁿ，无任何非 (Z/2) 因子；R8 = 256 = (Z/2)⁸ 是真闭合** | refactor pending |

A-F 是 native_decide 可证；G 涉及连续函数；H-K 是结构性扩展，落到 Lean 的 `Cell128.lean` (R7) + `Cell256.lean` (R8) + `Cell256Stratify.lean`。
...
| R6 quadrant partition | [`BenZheng.lean`](../../formal/SSBX/Foundation/Bagua/BenZheng.lean) | `quadrant_partition_complete`, `zong_quadrant` | ✓ |
| R3 phase plane + V₄ 同构 | 待落 (Theorem C/D 形式化) | `phase_eq`, `V₄_acts_on_phase` | TODO |

### 15.2 R7/R8 落地（refactor pending）
...
- **`Cell256.lean`**（替代 Cell192.lean，R8 闭合层）：
  ...
  /-- Shi V₄ 用户友好 inductive (4 ctors); 双射 to (因, 果) ∈ Bool². -/
  inductive Shi : Type
    | dao | ji | jin | wei
