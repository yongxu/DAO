...
10. **Item (c) — F₂-forcing chain (partial discharge).** [`Foundation/Wen/F2Forcing.lean`](../../formal/SSBX/Foundation/Wen/F2Forcing.lean) (`0` sorry, ~250 LOC) gives the §8.4 Stone–Birkhoff chain scaffold. Steps **2, 3, 5 are proven theorems**; steps 1 and 4 are `Prop` scaffolds with full proof sketches (Mathlib doesn't package Lindenbaum-Tarski or finite-Birkhoff as named decls).
    * **Step 2** — `BooleanAlgebra → BooleanRing` via Mathlib's `BooleanAlgebra.toBooleanRing` (Stone 1936). Discharged.
