...
10. **Item (c) — F₂-forcing chain (chain assembled).** [`Foundation/Wen/F2Forcing.lean`](../../formal/SSBX/Foundation/Wen/F2Forcing.lean) (`0` sorry, ~280 LOC) gives the §8.4 Stone–Birkhoff chain with all five steps as **real theorems** at the level needed by `UGCandidateFace.bitsEquiv` (which consumes only a type equivalence, not a BA-preserving iso).
    ...
    * **Step 4** — `step4_holds : ∀ U, step4_birkhoff U` via `Fintype.equivOfCardEq` (type-level Birkhoff). The BA-preserving refinement `step4_birkhoff_BAiso` remains scaffold — Mathlib's Birkhoff embedding for finite BAs (in `Mathlib.Order.Birkhoff`) requires linking sup-irreducibles ↔ atoms and a counting argument; ~100–200 LOC of additional engineering.
