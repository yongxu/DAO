...
- R5/R6 优先古字与古文语法；`力/作/徑/契` 这类能读通的字不降级。结构名如 `內本·動`、`本征·動` 负责消歧和 parser fallback。
...
R5 审计修正（2026-05-13）：公开默认入口恢复为下表 32 个单字。`力/作/徑/契` 等属于古字/古文语法可以直接读通的好名字；组合名 `內本·R4Name / 內征·R4Name` 只作无歧义 fallback。
...
推荐：`R7 = 因 = 重 × 因位`，`因位 ∈ {無因, 有因}`；公开 cell 读法用 `無跡 / 有跡`。

| 项 | 推荐 | 备选 | 取舍 |
|---|---|---|---|
| 层名 / 位名 | 因 | 緣 / 故 / 由 / 本 | `因` 能读作前因、条件、past trace，且和 `果` 成对。 |
| 位值 | 無跡 / 有跡；formal fallback 無因 / 有因 | 素 / 跡；未印 / 已印 | `無因/有因` 容易被当代读者读成“没有原因/有原因”，不如 `無跡/有跡` 直接；`素` 太泛，降为历史候选。 |
| 算子 | 印 | 記 / 留 / 銘 / 痕 | `印` 是留下印记，适合作 toggle 因的 action。 |

R7 reader grammar：

```text
R7Name ::= 無跡·R6Name | 有跡·R6Name | 無因·R6Name | 有因·R6Name
```
...
审计修正（2026-05-13）：R8 默认用 `Shi·卦名` 前置状态结构；`乾道 / 乾未 / 乾已 / 乾今` 这类紧凑写法只作短 alias。前置结构对当代读者更直接，因为它先说明 Shi 状态，再说明承载的 R6 卦。
