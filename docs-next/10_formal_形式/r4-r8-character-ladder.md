...
推荐：`R7 = 因 = 重 × 因位`，`因位 ∈ {無因, 有因}`；公开 cell 字用 `素 / 跡` 作可读 alias。

| 项 | 推荐 | 备选 | 取舍 |
|---|---|---|---|
| 层名 / 位名 | 因 | 緣 / 故 / 由 / 本 | `因` 能读作前因、条件、past trace，且和 `果` 成对。 |
| 位值 | 無因 / 有因；公开字 素 / 跡 | 未印 / 已印；本 / 迹 | state 不应混入 action；故不用「印」作位值；`素` 读未着迹，`跡` 读已有迹。 |
| 算子 | 印 | 記 / 留 / 銘 / 痕 | `印` 是留下印记，适合作 toggle 因的 action。 |

R7 reader grammar：

```text
R7Name ::= R7Glyph | 素·R6Name | 跡·R6Name | 無因·R6Name | 有因·R6Name
R7Glyph ::= R6Glyph 素 | R6Glyph 跡
```
