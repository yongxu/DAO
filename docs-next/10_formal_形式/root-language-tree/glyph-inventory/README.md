...
|---|---:|---:|---|
| [R5](R5.md) | 64 | 32 cell + 32 operator | 32 个 cell 实字 alias；operator 为 5 位 XOR mask 字组 |
| [R6](R6.md) | 128 | 64 cell + 64 operator | 64 个六十四卦名；operator 为 6 位 XOR mask 字组 |
| [R7](R7.md) | 256 | 128 cell + 128 operator | 64 卦 × 素/跡；operator 加 `印` |
| [R8](R8.md) | 512 | 256 cell + 256 operator | 64 卦 × 道/未/已/今；operator 加 `印/投` |

读表原则：

- `推荐字词` 是当前公开入口；`无歧义读法` 给 parser / reader fallback。
- `字源状态` 明确区分既有卦名、实字 alias、Shi 位字组合、XOR 算子字组。
