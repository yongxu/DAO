...
> Lean 事实以 [BaguaAlgebra.lean](../../formal/SSBX/Foundation/Bagua/BaguaAlgebra.lean) / [Cell192.lean](../../formal/SSBX/Foundation/Bagua/Cell192.lean) / [Yi.lean](../../formal/SSBX/Foundation/Yi/Yi.lean) / [Yuan.lean](../../formal/SSBX/Foundation/Core/Yuan.lean) / [MonadRoot.lean](../../formal/SSBX/Foundation/Core/MonadRoot.lean) / [JianOntology.lean](../../formal/SSBX/Foundation/Jian/JianOntology.lean) 为准。
...
R4  六十四卦   (64)   canonical 序卦传名（不重命名，列冲突表）
        ...
        × Shi {已 / 今 / 未}
R5  Cell192    (192)  cell 称 = 卦 + 时（"乾今" 等）
                       shi 转 迁 / 溯 ⭐
                       setShi = 置
                       cell 级算子 = R4 算子字 + 格
...
# R5：Cell192（= Hexagram × Shi）

R5 把 R4 的 64 卦乘上 3 个时态状态，得到 192 个"卦时格"。

## R5 时态元素 Shi（3 个）

| 元素 | Lean | literal | 推荐理由 | 主要备选（每个理由） |
|---|---|---|---|---|
| 已（过去） | `Shi.ji` | `已` | 现代汉语 `已经 / 已然 / 已往` 通用；古义"已往"也通；不撞 | `既`（古义"既往"，alias）/ `往`（动作向，alias）/ `过`（现代但太宽）/ `先`（"先前"alias）|
| 今（现在） | `Shi.jin` | `今` | 现代汉语 `今天 / 现今 / 如今` 通用；古义即"现在"；不撞 | `现`（现代直接，alias）/ `见`（古义"现身"，偏冷）/ `当`（"当下"alias）/ `时`（太宽）|
| 未（未来） | `Shi.wei` | `未` | 现代汉语 `未来 / 未济` 直接；64 卦最末"未济"自然呼应；不撞 | `将`（"将来"，alias）/ `来`（"来日"，alias）/ `后`（"后来"，alias）/ `未来` 双字（alias）|

`已 / 今 / 未` 都是单字、现代通用、传统也用、互不撞——**这一组保持不动**。

## R5 时态转换算子 ⭐ 新定

| Lean | 作用 | 推荐 | 推荐理由 |
|---|---|---|---|
| `Shi.next` / `shiNext` | 已→今→未→已 | **`迁`** ⭐ | "时迁"古义为时间推进；现代汉语 `变迁 / 迁移 / 迁就` 通用；不撞 CoreAtom |
| `Shi.prev` / `shiPrev` | 已←今←未←已 | **`溯`** ⭐ | "溯往"古义为追溯过去；现代汉语 `溯源 / 追溯 / 上溯` 通用；与 `迁`（前推）形成方向对待；不撞 |
...
|---|---|---|
| `hexCuo` 提升 | Cell192 全反 | **`错格`**（双字） |
| `hexZong` 提升 | Cell192 反序 | **`综格`**（双字） |
| `hexHu` 提升 | Cell192 互 | **`互格`**（双字） |
| `flip1..6` 提升 | Cell192 单爻翻转 | **`改格 / 化格 / 变格 / 临格 / 主格 / 极格`**（双字） |
...
L0 是 Cell192 上的受控虚拟机。candidates §"L0 BaguaWen 指令锚点" 已基本 settled，下面把 default 字按"现代汉字 + 双字必要时"原则展开：
...
| 事 | 本体读法第四阶段 ≈ R5 Cell192 transition | 隐含于 [`Cell192.lean`](../../formal/SSBX/Foundation/Bagua/Cell192.lean) 的 transition | 事 = "关系在某一时态/位点上发生"≈ Cell192 之间的一条 transition | ✓ canonical 单字（未在 CoreAtom 列表，但作 conceptual anchor）|
...
| 事 | **`事`** | 关系在时态 / 位点上发生 | ≈ R5 Cell192 之间的 transition |
...
|---|---|
| 推荐 | **`事`** |
| 形式锚 | ≈ R5 Cell192 之间的 transition（"事之发生" = 卦时格之间的迁移）|
...
- 在 [root-layer-map.md §0](root-layer-map.md) 已确立"事 ≈ Cell192 transition"的读法
...
观察：`事` 在系统里多次作为 conceptual anchor 出现（本体读法、Cell transition、内容线下游），但目前未 register。建议在下一轮 CoreAtom roster 评审时考虑提升——尤其因为 [Cell192](../../formal/SSBX/Foundation/Bagua/Cell192.lean) 的 transition 已 theorem-backed，`事` 可作为它的 surface alias。这是**待办项**（详 §I）。
...
- [ ] `事` 是否提升为 [CoreAtom](../../formal/SSBX/Foundation/Core/MonadRoot.lean)？理由：Cell192 transition 已 theorem-backed，`事` 可作 surface anchor
...
## 阶段 2：R4 单爻 + Cell192 ⭐ 含 临 / 主 新决定
```
临 / 主 / 极  (R4 flip4/5/6 outer)
极 / 错格 / 综格 / 互格 / 改格 / 化格 / 变格 / 临格 / 主格 / 极格  (Cell)
```

## 阶段 3：R5 时态算子 ⭐ 含 迁 / 溯 新决定
```
迁 / 溯  (R5 shiNext / shiPrev)
置  (setShi)
```
...
candidates 没给 shi 转换字。本表选 `迁 / 溯`。

**缓解**：`迁` 与 4 爻位"进"alias 微撞但分层；`溯` 完全干净。如果未来需要切换，候选 `进 / 退` 或 `推 / 拉`（推已给 .push）。
...
| [Cell192.lean](../../formal/SSBX/Foundation/Bagua/Cell192.lean) | 本表 R5 Shi / cell 算子的 ground truth。 |
...
- [ ] R5 `迁 / 溯` 在 OperatorAnchors 注册（独立任务）
