...
[`Cell256.lean`](../../formal/SSBX/Foundation/Bagua/Cell256.lean) § 1（Phase C, 2026-05-11 后）：

```lean
abbrev Shi : Type := YinBit × GuoBit   -- = Bool × Bool

namespace Shi

@[match_pattern] def dao : Shi := (false, false)  -- 道 (V₄ identity)
@[match_pattern] def ji  : Shi := (true,  false)  -- 已 (P)
@[match_pattern] def jin : Shi := (true,  true)   -- 今 (PT)
@[match_pattern] def wei : Shi := (false, true)   -- 未 (T)

def all : List Shi := [dao, ji, jin, wei]
theorem all_length : all.length = 4 := rfl
```

**Phase C 注**：Phase C (commit 90c34f0) 前 Shi 是 `inductive Shi { dao, ji, jin, wei }`；Phase C 后改为 `abbrev` of `Bool × Bool`，4 个名字下放为 `@[match_pattern] def`s。Pattern 语法 `match s with | Shi.dao => ...` 仍工作；V₄ 群结构现在 literally = componentwise XOR 之 `Bool × Bool`。
