...
- `ClassicalTextRHierarchyBridge.lean`、`MetaInterp/Assembly.lean`、`MetaInterp/Universal.lean`、`MetaInterp/Fetch.lean`、`MetaInterp/FetchProg.lean` 已给 classical-language direct bridge + base-256 structural assembly + zero-step/prologue compose + exact-fuel semantic frontier + U.1/U.2/U.3 obligation interface + pc=0 fetch route + pc-counter peel bridge + concrete fetch-with-peel splice + exact halted/running fetch routes + dispatch segment witness + all-op absolute-pc dispatch routes；不宣称 full universality
