...
| F.7c.31 exact block witnesses | compatibility blocks may prove `BlockPre → BlockPost`, but arbitrary-current blocks should use `CarryBlockPre`: runtime `META.cur = regDataCell regHex`, while `R₄ × R₄ carry` preserves `sim.cur`; current-only blocks need executable pc-counter mutation, stack blocks need middle-history mutation, control blocks need pc rewrite | `R4CarryBlockContracts.R4CarryExactBlockContracts`, `R4CarryBlockPrePost.R4CarryIndexedBlockContracts`, `ExecuteBlock.blockPost_unchanged_history_impossible_after_pc_increment` |
