...
| 13 | **Base-256 self-interpreter structural assembly**：concrete `metaInterpProg` has fixed length, outer-loop fetch entry, concrete fetch segment witness, assembly-specialized fetch route wrappers, and halt arm structural witness | `proved` (typed skeleton) | `machineChecked` | `MetaInterp/Assembly.lean` `metaInterpProg_base256_structural_summary`, `metaInterpProg_fetchProg_at_offset`, `metaInterpProg_fetch_routes_running_to_dispatch_at_fuel` |
...
| # | claim | status | 备注 |
|---|---|---|---|
| 16 | **Classical-language self-interpreter full semantic universal-compose theorem** | `pending` | embedded prologue U.0a、zero-step/prologue compose、exact-fuel U.0 composition frontier、U.1/U.2/U.3 obligation interface、generic META halted padding、Strategy-B fixed-parameter boundary、pc=0 fetch exact dispatch route、`FetchProg` peel-witness exact-fuel routes 与 assembly-specialized fetch route wrappers 已 machineChecked；仍缺从 general concrete fetch peel/decode、hard-block semantic effects、parameterized sub-dispatch 构造 `SemanticLoopObligations`，以及 arbitrary-program simulation。详 `MetaInterp/Universal.lean` / `MetaInterp/Fetch*.lean` / `MetaInterp/Assembly.lean`。 |
...
- [`formal/SSBX/Foundation/Wen/MetaInterp/Assembly.lean`](../../formal/SSBX/Foundation/Wen/MetaInterp/Assembly.lean) — `metaInterpProg_base256_structural_summary`, `metaInterpProg_fetchProg_at_offset`, `metaInterpProg_fetch_routes_running_to_dispatch_at_fuel`
