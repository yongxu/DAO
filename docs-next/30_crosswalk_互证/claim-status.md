...
| 13 | **Base-256 self-interpreter structural assembly**：concrete `metaInterpProg` has fixed length, outer-loop fetch entry, concrete fetch/dispatch segment witnesses, assembly-specialized fetch route wrappers, halt-arm absolute-pc dispatch route, and 7-fuel halt execution smoke witness | `proved` (typed skeleton) | `machineChecked` | `MetaInterp/Assembly.lean` `metaInterpProg_base256_structural_summary`, `metaInterpProg_fetchProg_at_offset`, `metaInterpProg_dispatchProg_at_offset`, `metaInterpProg_fetch_routes_running_to_dispatch_at_fuel`, `metaInterpProg_dispatch_routes_halt_at_fuel`, `metaInterpProg_dispatch_halt_executes_halt` |
