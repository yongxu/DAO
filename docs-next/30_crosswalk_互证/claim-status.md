...
| 13 | **Base-256 self-interpreter structural assembly**：concrete `metaInterpProg` has fixed length, outer-loop fetch entry, concrete fetch/dispatch segment witnesses, assembly-specialized fetch route wrappers, and halt arm structural witness | `proved` (typed skeleton) | `machineChecked` | `MetaInterp/Assembly.lean` `metaInterpProg_base256_structural_summary`, `metaInterpProg_fetchProg_at_offset`, `metaInterpProg_dispatchProg_at_offset`, `metaInterpProg_fetch_routes_running_to_dispatch_at_fuel` |
