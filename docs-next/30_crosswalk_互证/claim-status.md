...
| # | claim | status | 备注 |
|---|---|---|---|
| 16 | **Classical-language self-interpreter full semantic universal-compose theorem** | `pending` | embedded prologue U.0a、zero-step/prologue compose、exact-fuel U.0 composition frontier、U.1/U.2/U.3 obligation interface、generic META halted padding、Strategy-B fixed-parameter boundary、pc=0 fetch exact dispatch route、`FetchProg` peel-witness exact-fuel routes、assembly-specialized fetch route wrappers、dispatch segment witness、halt-arm absolute-pc dispatch route 与 halt execution smoke witness 已 machineChecked；仍缺从 general concrete fetch peel/decode/restore、all-op dispatch absolute-pc route lift、hard-block encoded-history semantic effects、parameterized sub-dispatch 构造 `SemanticLoopObligations`，以及 arbitrary-program simulation。详 `MetaInterp/Universal.lean` / `MetaInterp/Fetch*.lean` / `MetaInterp/Assembly.lean`。 |
