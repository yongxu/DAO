...
| 轴 | 状态 | 说明 |
|---|---|---|
| amplitude dynamics 与物理测量语义 | 未纳入本轮 | S23 已关闭 finite period-two phase operation，S24 已关闭 displayed continuous action-coordinate sampling，S25 已关闭 finite path-space action handoff，S26 已关闭 finite action extremum boundary；S27-S29 补的是 finite causal locality、displayed two-step interval 与 kernel path carrier，不是 amplitude dynamics。仍未证明 Hamiltonian generator、continuous-time unitary group、general Schrödinger dynamics、measurement postulate semantics、decoherence 或无条件物理测量概率律 |
...
| 因果局部性 | 下一步只依赖局部过去或邻域 | S27 已关闭 one-step localFuture；S28 已关闭 displayed two-step interval；S29 已关闭 finite kernel path carrier |
