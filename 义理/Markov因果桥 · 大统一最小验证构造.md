...
| 文构造覆盖修正 | `Foundation/Modern/QuantumRelativityWenBoundary.lean` | `wen_constructive_boundary_summary` 证明 `256 × 371` 覆盖不受 tagged 边界否定 | `machineChecked` |
...
| `94976` grid bridge (v3：原 `94976` ) | `Foundation/Modern/OperatorCellGridMarkovBridge.lean` | `operator_cell_grid_markov_causal_bridge_summary` 把 `371 × 256` 覆盖网格作为有限过程状态空间 (v3：原 `371 × 256`，Cell192→Cell256 重号) | `machineChecked` |
...
| S16 sum-over-middle Born 边界候选 | `Foundation/Modern/QuantumRelativitySumOverMiddleBornBoundaryBridge.lean` | `sum_over_middle_born_distribution_bridge_summary` 关闭 composed endpoint support 到 finite Born distribution boundary 的条件式接口 | `machineChecked` |
| S17 unitary/CPTP 账本边界 | `Foundation/Modern/QuantumRelativityUnitaryCPTPLedgerBridge.lean` | `unitary_cptp_ledger_bridge_summary` 关闭 current skeleton / physical ledger boundary | `machineChecked` |
| S18 Born rule 推导候选 | `Foundation/Modern/QuantumRelativityBornRuleDerivationBridge.lean` | `born_rule_derivation_bridge_summary` 关闭 Markov/amplitude compatibility 下的 finite typed-skeleton derivation | `machineChecked` |
| S19 路径权重乘法候选 | `Foundation/Modern/QuantumRelativityPathWeightMultiplicationBridge.lean` | `path_weight_multiplication_bridge_summary` 关闭 finite kernel path 的 append weight law | `machineChecked` |
| S20 非平凡量子通道律候选 | `Foundation/Modern/QuantumRelativityNontrivialChannelLawBridge.lean` | `nontrivial_quantum_channel_law_bridge_summary` 关闭 finite support-respecting nonzero channel law | `machineChecked` |
| S21 概率幅到测量权重候选 | `Foundation/Modern/QuantumRelativityBornMeasurementBridge.lean` | `born_measurement_bridge_summary` 关闭 one-qubit computational-basis measurement weights | `machineChecked` |
| S22 作用相位到测量权重链 | `Foundation/Modern/QuantumRelativityActionAmplitudeMeasurementBridge.lean` | `action_amplitude_measurement_bridge_summary` 关闭 finite action branch 到 measurement-event weights 的推出链 | `machineChecked` |
| S23 有限相位演化候选 | `Foundation/Modern/QuantumRelativityFinitePhaseEvolutionBridge.lean` | `finite_phase_evolution_bridge_summary` 关闭 finite phase evolution 到 Born-weight preservation 的边界 | `machineChecked` |
| S24 连续作用量泛函候选 | `Foundation/Modern/QuantumRelativityContinuousActionFunctionalBridge.lean` | `continuous_action_functional_bridge_summary` 关闭 displayed continuous action-coordinate functional 到 sampled phase evolution 的边界 | `machineChecked` |
| S25 路径空间作用量泛函候选 | `Foundation/Modern/QuantumRelativityPathSpaceActionFunctionalBridge.lean` | `path_space_action_functional_bridge_summary` 关闭 finite quotient path space action functional 到 action phase / sampling / cancellation 的边界 | `machineChecked` |
| S26 有限作用量极值候选 | `Foundation/Modern/QuantumRelativityFiniteActionExtremumBridge.lean` | `finite_action_extremum_bridge_summary` 关闭 finite action gap / minimum / non-minimum boundary | `machineChecked` |
| S27 有限因果局部性候选 | `Foundation/Modern/QuantumRelativityFiniteCausalLocalityBridge.lean` | `finite_causal_locality_bridge_summary` 关闭 finite localFuture / one-step support locality boundary | `machineChecked` |
| S28 有限因果区间候选 | `Foundation/Modern/QuantumRelativityFiniteCausalIntervalBridge.lean` | `finite_causal_interval_bridge_summary` 关闭 displayed two-step causal interval / localFuture handoff boundary | `machineChecked` |
| S29 有限核路径载体候选 | `Foundation/Modern/QuantumRelativityFiniteKernelPathCarrierBridge.lean` | `finite_kernel_path_carrier_bridge_summary` 关闭 finite KernelPath carrier / path-local interval soundness boundary | `machineChecked` |
| S30 递归核路径载体候选 | `Foundation/Modern/QuantumRelativityKernelPathRecursiveCarrierBridge.lean` | `kernel_path_recursive_carrier_bridge_summary` 关闭 constructor-recursive carrier / append readback boundary | `machineChecked` |
| S31 端点索引递归载体族候选 | `Foundation/Modern/QuantumRelativityEndpointIndexedRecursiveCarrierBridge.lean` | `endpoint_indexed_recursive_carrier_bridge_summary` 关闭 endpoint-indexed recursive carrier family boundary | `machineChecked` |
| S32 双路径递归载体族完备候选 | `Foundation/Modern/QuantumRelativityTwoRouteDisplayedCarrierCompletenessBridge.lean` | `two_route_displayed_carrier_completeness_bridge_summary` 关闭 two-route displayed family completeness boundary | `machineChecked` |
| S33 双路径 KernelPath 枚举候选 | `Foundation/Modern/QuantumRelativityTwoRouteKernelPathEnumerationBridge.lean` | `two_route_kernel_path_enumeration_bridge_summary` 关闭 two-route all-KernelPath carrier enumeration boundary | `machineChecked` |
...
最小桥接尝试 =
  ...
  + sum-over-middle Born 边界候选 witness
  + unitary/CPTP required ledger witness
  + Born rule from Markov/amplitude compatibility witness
  + path-weight multiplication witness
  + nontrivial finite quantum-channel law witness
  + one-qubit Born measurement weights witness
  + action branch to measurement-event weights witness
  + finite phase evolution witness
  + displayed continuous action functional witness
  + finite path-space action functional witness
  + finite action extremum witness
  + finite causal locality witness
  + finite causal interval witness
  + finite kernel path carrier witness
  + recursive kernel path carrier witness
  + endpoint-indexed recursive carrier family witness
  + two-route displayed carrier completeness witness
  + 仍服从 tagged physical-language noncollapse
  + 不否定 256 × 371 文构造覆盖
```

当前闭合的是有限 typed-skeleton 层：它把上一层的“中介桥方向”推进为一个候选最小构造。同一个有限过程对象可以被双读，终端状态可以同时被读为测量结果与事件记录，非终端 Markov 行可带有限分母候选，显式 path witness 可以组合成可达 / 因果读法，classical Markov 层与 quantum amplitude/channel candidate 层被形式地区分，并且 path amplitude、相消 witness、Born-shaped boundary、非零 path-amplitude witness、two-path finite cancellation witness、discrete phase-label witness、edge-action phase accumulation witness、finite path-family sum witness、finite path-sum algebra witness、endpoint-indexed finite family witness、endpoint support normalization witness、two-route toy enumeration witness、visible path-key witness、finite visible-key quotient candidate、visible-key quotient class witness、two-route canonical representative witness、finite quotient-support witness、quotient-support algebra witness、pending observable ledger witness、finite action-to-phase law witness、finite row sum-one normalization witness、normalized mass sum-one witness、conditional Born-weight witness、finite Born distribution witness、pointwise channel composition witness、sum-over-middle two-step composition witness、sum-over-middle Born boundary witness、unitary/CPTP required ledger witness、Born rule derivation witness、path-weight multiplication witness、nontrivial finite quantum-channel law witness、one-qubit computational-basis Born measurement weights witness、action branch to measurement-event weights witness、finite phase evolution witness、displayed continuous action functional witness、finite path-space action functional witness、finite action extremum witness、finite causal locality witness、finite causal interval witness、finite kernel path carrier witness 与 recursive kernel path carrier witness 可以作为候选接口被记录；S8 把这些已关闭结构的当前核心合取为 stepwise summary theorem，S9 继续关闭 finite Markov row normalization boundary，S10 把 row-total candidate 改写成逐项 rational probability mass law，S11 在 amplitude support 已归一条件下关闭 candidateWeight 非负与 sum-one，S12 把这些对象打包为 finite probability distribution interface，S13 给 current channel skeleton 增加 `channelCompose`，S14 关闭 pointwise associativity 与 diagonal identity obstruction，S15 关闭 finite middle-list composition 的 two-step boundary，S16 关闭 composed endpoint support 到 finite Born distribution boundary 的条件式接口，S17 关闭 current skeleton / physical channel-law ledger boundary，S18 关闭 Markov/amplitude compatibility 下的 finite typed-skeleton Born-rule derivation，S19 关闭 finite kernel path 的 path-weight multiplication law，S20 关闭 finite support-respecting nonzero channel law，S21 关闭 one-qubit computational-basis normalized-state measurement weights，S22 关闭 finite action branch -> action amplitude -> normalized qubit -> measurement-event weights chain，S23 关闭 branch-indexed finite phase evolution -> S22 qubits -> Born-weight preservation boundary，S24 关闭 displayed continuous action-coordinate functional `S(t)=t` -> branch samples -> finite phase evolution -> Born preservation boundary，S25 关闭 finite quotient path space action functional -> S5r action phase -> S24 sampling -> quotient-support cancellation boundary，S26 关闭 two-route quotient support 上的 action gap、upper finite minimum 与 lower non-minimum boundary，S27 关闭 finite localFuture list / one-step support locality boundary，S28 关闭 displayed two-step causal interval / localFuture handoff boundary，S29 关闭 finite kernel path displayed carrier / path-local interval soundness boundary，S30 关闭 constructor-recursive kernel path carrier / append carrier readback boundary，并显式列出仍未关闭边界。

S31 进一步关闭 endpoint-indexed recursive carrier family boundary：two-route 的 recursive carriers 被包装为同端点 finite family，可读回 carrier lists、weights `[1, 1]` 与 displayed weight sum `2`，但仍不声称 all-path completeness。

S32 关闭的是更窄的 two-route displayed family completeness：该 displayed family 恰好包含 upper/lower 两个 recursive carriers，任意 displayed member 都落在这两者之一，且权重为 `1`；这仍不是 all KernelPath enumeration。

S33 关闭 twoRoute toy source/target 的 all-`KernelPath` carrier enumeration：任意 `KernelPath twoRouteKernel source target` 的 `kernelPathPoints` 只能是 `[source, upper, target]` 或 `[source, lower, target]`，并且权重为 `1`，可读回 S32 displayed family；这仍不推广到任意有限过程。
...
| `256 × 371` 文构造覆盖 | `machineChecked` | 复用 `wen_constructive_boundary_summary`；它不是 tagged 边界的否定对象 |
...
| `94976` operator-cell grid bridge | `machineChecked` | `operator_cell_grid_markov_causal_bridge_summary` 将 `256 × 371` 覆盖网格接入 `FiniteProcess` |
...
| sum-over-middle Born boundary | `machineChecked` theorem | S16 证明 composed endpoint amplitude support 若已归一，则接入 finite Born distribution boundary；concrete support 为 `[1]` |
| unitary/CPTP ledger boundary | `machineChecked` theorem | S17 证明当前 skeleton items 已关闭，同时 physical channel law items required but not closed |
| Born rule 从 Markov / 振幅桥的推导 | `machineChecked` theorem | S18 在 `MarkovAmplitudeCompatibility` 假设下证明 normalized amplitude support，并接入 S12 `BornDistributionBoundary` |
| 路径权重乘法定律 | `machineChecked` theorem | S19 证明 finite kernel path append 的 weight multiplication law，并保留旧 `pathWeight = 1` placeholder law |
| 非平凡有限量子通道律 | `machineChecked` theorem | S20 证明非零 channel amplitude 推出 process step、carried classical support 与 Born-shaped boundary，并给出 concrete 非零二步组合 |
| one-qubit Born measurement weights | `machineChecked` theorem | S21 证明 normalized qubit 的 `bornProb0` / `bornProb1` 非负且和为 `1` |
| action branch to measurement-event weights | `machineChecked` theorem | S22 证明 finite action branch 推出 action amplitude、normalized qubit 与二值 measurement-event weights |
| finite phase evolution | `machineChecked` theorem | S23 证明 branch-indexed `1/-1` phase operation 生成 S22 qubits，并保持 computational-basis Born weights / normalized measurement-weight boundary |
| displayed continuous action functional | `machineChecked` theorem | S24 证明 `S(t)=t` 连续，并在 branch samples 上回到 finite action index、phase amplitude 与 S23 Born-weight preservation |
| finite path-space action functional | `machineChecked` theorem | S25 证明 finite quotient path classes 上的 action values 接回 S5r action index、S24 samples 与 quotient-support cancellation |
| finite action extremum | `machineChecked` theorem | S26 证明 two-route quotient support 上 lower-minus-upper action gap 为 `1`，upper 是 finite action minimum，lower 不是 |
| finite causal locality | `machineChecked` theorem | S27 证明 finite localFuture list 精确覆盖 one-step support，并且 positive kernel weight respects localFuture |
| finite causal interval | `machineChecked` theorem | S28 证明 displayed two-step causal interval 的 middle list sound/complete，并 respect localFuture |
| finite kernel path carrier | `machineChecked` theorem | S29 证明 finite `KernelPath` 可附 displayed carrier，保留 weight / causal readback，并给出 sound two-step path-local interval |
| recursive kernel path carrier | `machineChecked` theorem | S30 证明 finite `KernelPath` 的 carrier 可由 constructors 递归生成，并给出 append carrier readback 与 two-route recursive carrier family |
| endpoint-indexed recursive carrier family | `machineChecked` theorem | S31 证明 recursive carriers 可包成同端点 finite family，并读回 carrier lists、weights 与 displayed weight sum |
| two-route displayed carrier completeness | `machineChecked` theorem | S32 证明 two-route displayed recursive family 的成员恰好是 upper/lower，且任意 displayed member 权重为 `1` |
| two-route all-KernelPath carrier enumeration | `machineChecked` theorem | S33 证明 twoRoute source/target 任意 `KernelPath` 的 recursive carrier 只能是 upper route 或 lower route，且权重为 `1` |
| 真实干涉、physical Hilbert/unitary/CPTP/Kraus/density-matrix channel law、一般测量语义 | 未纳入本轮 | S5/S5b-S5r 与 S13-S33 已开候选接口；还需要真正关闭 Hilbert/unitary/CPTP/Kraus/density-matrix、一般 Hilbert measurement、POVM/PVM、smooth/infinite-dimensional path-space action functional、continuous variation space、stationary action principle、stochastic semantics、general Hamiltonian/unitary amplitude dynamics、measurement postulate semantics、decoherence、general choice function、general all-path enumeration、一般 path integral 或可测预言 theorem |
...
| 文构造覆盖 | `wen_constructive_boundary_summary` | `256 × 371` 覆盖与 tagged 边界相容 |
...
| conditional Born weights | `born_weight_nonnegative_and_sum_one_if_normalized` | `machineChecked` theorem |
| S11 公开摘要 | `born_weight_normalization_bridge_summary` | `machineChecked` theorem |
| finite Born distribution | `BornDistributionBoundary`、`born_distribution_boundary` | `machineChecked` theorem |
...
| sum-over-middle Born boundary | `sumOverMiddleEndpointAmplitudeSupport`、`sum_over_middle_born_distribution_boundary` | `machineChecked` theorem |
| concrete composed Born input | `concrete_sumOverMiddle_endpoint_amplitude_support`、`concrete_composed_endpoint_amplitude_support_normalized` | `machineChecked` theorem |
| S16 公开摘要 | `sum_over_middle_born_distribution_bridge_summary` | `machineChecked` theorem |
| unitary/CPTP ledger | `CurrentChannelBoundaryItem`、`UnitaryCPTPLedgerItem` | `machineChecked` typed skeleton |
| unitary/CPTP required pending | `unitary_cptp_ledger_required_but_not_closed_by_s17` | `machineChecked` theorem |
| S17 公开摘要 | `unitary_cptp_ledger_bridge_summary` | `machineChecked` theorem |
| Markov/amplitude compatibility | `MarkovAmplitudeCompatibility` | `machineChecked` typed skeleton |
| Born rule derivation from Markov/amplitude bridge | `markov_amplitude_born_rule_derivation`、`concrete_prepared_born_rule_from_markov_amplitude_bridge` | `machineChecked` theorem |
| S18 公开摘要 | `born_rule_derivation_bridge_summary` | `machineChecked` theorem |
| path-weight multiplication | `KernelPath.weight_append`、`concrete_two_step_path_weight_multiplicative` | `machineChecked` theorem |
| S19 公开摘要 | `path_weight_multiplication_bridge_summary` | `machineChecked` theorem |
| nontrivial finite quantum-channel law | `nontrivial_channel_amplitude_law`、`concrete_nontrivial_sum_over_middle_channel_law` | `machineChecked` theorem |
| S20 公开摘要 | `nontrivial_quantum_channel_law_bridge_summary` | `machineChecked` theorem |
| one-qubit Born measurement weights | `born_rule_gives_normalized_measurement_weights`、`computational_basis_measurement_weights_normalized` | `machineChecked` theorem |
| S21 公开摘要 | `born_measurement_bridge_summary` | `machineChecked` theorem |
| finite action-to-measurement chain | `action_branch_outcome_weights_normalized`、`action_to_born_measurement_chain_closed` | `machineChecked` theorem |
| S22 公开摘要 | `action_amplitude_measurement_bridge_summary` | `machineChecked` theorem |
| finite phase evolution | `phase_evolution_reaches_action_branch_qubits`、`phase_evolve_branch_measurement_weights_normalized` | `machineChecked` theorem |
| S23 公开摘要 | `finite_phase_evolution_bridge_summary` | `machineChecked` theorem |
| displayed continuous action functional | `displayedContinuousActionFunctional`、`displayed_continuous_action_functional_continuous` | `machineChecked` theorem |
| continuous action sampling | `branch_continuous_action_value_matches_index`、`continuous_action_sampled_amplitude_eq_branch_amplitude` | `machineChecked` theorem |
| continuous action to Born preservation | `continuous_action_phase_evolve_eq_finite_phase_evolve`、`continuous_action_phase_evolve_measurement_weights_normalized` | `machineChecked` theorem |
| S24 公开摘要 | `continuous_action_functional_bridge_summary` | `machineChecked` theorem |
| finite path-space action functional | `twoRoutePathSpaceActionFunctional`、`two_route_upper_path_space_action_value`、`two_route_lower_path_space_action_value` | `machineChecked` theorem |
| path-space action compatibility | `two_route_path_space_action_index_matches_action_phase_law`、`branch_path_space_action_value_eq_continuous_action_value` | `machineChecked` theorem |
| path-space action cancellation | `two_route_path_space_action_support_amplitude_cancels`、`two_route_path_space_action_support_born_weight_zero` | `machineChecked` theorem |
| S25 公开摘要 | `path_space_action_functional_bridge_summary` | `machineChecked` theorem |
| finite action gap | `two_route_upper_to_lower_action_gap`、`two_route_lower_to_upper_action_gap` | `machineChecked` theorem |
| finite action minimum | `two_route_upper_is_finite_action_minimum`、`two_route_lower_not_finite_action_minimum` | `machineChecked` theorem |
| S26 公开摘要 | `finite_action_extremum_bridge_summary` | `machineChecked` theorem |
| finite localFuture | `FiniteCausalLocalFutureCandidate`、`local_future_iff_step` | `machineChecked` theorem |
| kernel locality | `KernelRespectsLocalFuture`、`kernel_positive_weight_implies_local_future` | `machineChecked` theorem |
| S27 公开摘要 | `finite_causal_locality_bridge_summary` | `machineChecked` theorem |
| finite two-step interval | `FiniteTwoStepCausalIntervalCandidate`、`interval_respects_local_future` | `machineChecked` theorem |
| S28 公开摘要 | `finite_causal_interval_bridge_summary` | `machineChecked` theorem |
| finite kernel path carrier | `DisplayedKernelPathCarrier`、`SoundKernelTwoStepPathLocalIntervalCandidate` | `machineChecked` theorem |
| S29 公开摘要 | `finite_kernel_path_carrier_bridge_summary` | `machineChecked` theorem |
| recursive kernel path carrier | `kernelPathTargets`、`kernelPathPoints`、`recursiveKernelPathCarrier` | `machineChecked` theorem |
| S30 公开摘要 | `kernel_path_recursive_carrier_bridge_summary` | `machineChecked` theorem |
| endpoint-indexed recursive carrier family | `EndpointIndexedRecursiveKernelCarrierFamily`、`twoRouteEndpointIndexedRecursiveCarrierFamily` | `machineChecked` theorem |
| S31 公开摘要 | `endpoint_indexed_recursive_carrier_bridge_summary` | `machineChecked` theorem |
| two-route displayed carrier completeness | `TwoRouteDisplayedRecursiveCarrierFamilyComplete`、`two_route_displayed_recursive_family_member_cases` | `machineChecked` theorem |
| S32 公开摘要 | `two_route_displayed_carrier_completeness_bridge_summary` | `machineChecked` theorem |
| two-route all-KernelPath carrier enumeration | `TwoRouteKernelPathEnumerationComplete`、`two_route_kernel_path_source_target_points_cases` | `machineChecked` theorem |
| S33 公开摘要 | `two_route_kernel_path_enumeration_bridge_summary` | `machineChecked` theorem |

未纳入本轮：

| 轴 | 状态 | 说明 |
|---|---|---|
| amplitude dynamics 与物理测量语义 | 未纳入本轮 | S23 已关闭 finite period-two phase operation，S24 已关闭 displayed continuous action-coordinate sampling，S25 已关闭 finite path-space action handoff，S26 已关闭 finite action extremum boundary；S27-S33 补的是 finite causal locality、displayed two-step interval、kernel path carrier、recursive carrier、endpoint-indexed carrier family、two-route displayed completeness 与 two-route all-KernelPath carrier enumeration，不是 amplitude dynamics。仍未证明 Hamiltonian generator、continuous-time unitary group、general Schrödinger dynamics、measurement postulate semantics、decoherence 或无条件物理测量概率律 |
| 一般 Hilbert measurement / POVM / PVM | 未纳入本轮 | S21/S22/S23 只关闭 one-qubit computational-basis normalized-state weights、finite action branch 接入与 finite phase preservation；未引入一般 Hilbert carrier、projection-valued measures 或 positive operator-valued measures |
| stochastic semantics / all-path enumeration | 未纳入本轮 | S19 已关闭 finite kernel path 的 weight append law；仍未证明概率独立语义、任意路径枚举或一般 path integral |
| physical unitary/CPTP/Kraus/density-matrix law | 未纳入本轮 | S20 已关闭 finite support-respecting nonzero channel law；仍未证明 Hilbert carrier、unitary evolution、CPTP、Kraus representation 或 density-matrix semantics |
| 真实干涉律 | 未纳入本轮 | S5-S5r 与 S24-S29 有相消 witness、action/function/extremum/locality/interval/carrier 等候选边界；仍没有 smooth/infinite-dimensional path-space action functional、general choice function、general all-path enumeration、一般 path integral、continuous-time dynamics 或可测预言 theorem |
| 完整因果集公理 | 未纳入本轮 | S3 只证明组合 witness 与一步 no-self-loop，S27-S33 只证明 localFuture、displayed two-step interval、kernel path carrier、recursive carrier、endpoint-indexed carrier family、two-route displayed completeness 与 two-route all-KernelPath carrier enumeration；不证明完整偏序、arbitrary-length causal interval、global local finiteness 或 manifold recovery |
| 度规与曲率 | 未纳入本轮 | 需要几何极限或额外重建结构 |
| 经验闭合 | 未纳入本轮 | S5q 已有 pending observable ledger entry；尚无外部数据、误差模型、阈值与数据判准 |

正面闭合范围：**Markov-因果桥的最小 typed skeleton、S2 有限概率核接口、S3 路径/因果约束、S4 Markov/amplitude-channel 分层候选、S5 interference/Born-shaped candidate、S5b nonzero path-amplitude candidate、S5c two-path finite cancellation candidate、S5d discrete phase-label candidate、S5e discrete edge-action phase candidate、S5f finite path-family sum candidate、S5g finite path-sum algebra candidate、S5h endpoint-indexed finite family candidate、S5i endpoint support normalization candidate、S5j two-route toy enumeration candidate、S5k visible path-key candidate、S5l finite visible-key quotient candidate、S5m visible-key quotient class candidate、S5n displayed canonical representative candidate、S5o finite quotient-support candidate、S5p quotient-support algebra candidate、S5q observable ledger candidate boundary、S5r finite action-to-phase law candidate、S8 stepwise summary、S9 finite row normalization、S10 normalized mass law、S11 conditional Born-weight law、S12 finite Born distribution boundary、S13 channel composition candidate、S14 channel associativity / identity obstruction、S15 sum-over-middle composition boundary、S16 composed Born boundary、S17 unitary/CPTP ledger boundary、S18 Born rule derivation boundary、S19 path-weight multiplication boundary、S20 nontrivial finite quantum-channel law boundary、S21 one-qubit Born measurement weights boundary、S22 finite action-to-measurement chain、S23 finite phase-evolution boundary、S24 displayed continuous action functional boundary、S25 finite path-space action functional boundary、S26 finite action extremum boundary、S27 finite causal locality boundary、S28 finite two-step causal interval boundary、S29 finite kernel path carrier boundary、S30 recursive kernel path carrier boundary、S31 endpoint-indexed recursive carrier family boundary、S32 two-route displayed carrier completeness boundary 与 S33 two-route all-KernelPath carrier enumeration boundary 已在 Lean 中关闭。它已经把有限过程双读、测量-事件对齐、同根非同一、tagged-language noncollapse 保持、非终端行分母非零、权重上界、finite row sum-one normalization、normalized mass sum-one law、conditional Born-shaped candidateWeight law、finite Born distribution packaging、Markov/amplitude compatibility 下的 Born rule derivation、one-qubit computational-basis measurement weights、finite action branch to measurement-event weights chain、finite phase evolution to Born-weight preservation、displayed continuous action-coordinate sampling、finite quotient path-space action handoff、finite action gap / minimum boundary、finite localFuture / one-step support boundary、displayed two-step causal interval boundary、finite kernel path carrier boundary、recursive kernel path carrier boundary、endpoint-indexed recursive carrier family boundary、two-route displayed carrier completeness boundary、two-route all-KernelPath carrier enumeration boundary、finite kernel path 的 weight append law、nonzero channel amplitude 到 step / classical support / Born-shaped boundary 的有限通道律、channelCompose candidate、pointwise associativity、diagonal identity obstruction、sum-over-middle two-step boundary、composed endpoint support 的条件式 Born distribution boundary、unitary/CPTP required-but-not-closed ledger、path witness 组合、一步 no-self-loop、layer separation、channel 到 S2 边界的投影、path amplitude candidate、相消 witness、`ampProb` boundary、非零候选振幅到 valid / Reachable / causalBefore 的边界、同端点不同中间态两路径候选相消、`zero/pi` 离散相位标签到 `1/-1` 的候选导出、edge phase increments 到 path phase 的累积、relative phase `pi`、edge-action-induced cancellation、finite same-endpoint path family sum、append / permutation / reverse stability、endpoint-indexed conversion、amplitude-complete filter preservation、duplicate handling boundary、toy source/target middle enumeration、visible path-key boundary、toy source/target key completeness、key-compatible amplitude descent、finite key duplicate compensation、two-route key-level cancellation、quotient class construction、two-route quotient completeness、displayed representative completeness、quotient-support cancellation、quotient-support algebraic stability、pending observable ledger boundary、finite action index law、current stepwise summary、finite row normalized total、finite classical probability law、conditional finite Born-weight probability law 与 current skeleton channel composition 都压成了可检查 theorem。**

尚未闭合的范围同样应实事求是列出：stochastic independence semantics、general all-path enumeration、smooth/infinite-dimensional path-space action functional、continuous variation space、stationary action principle、path integral、general Hamiltonian/unitary amplitude dynamics、general Hilbert measurement、POVM/PVM、measurement postulate semantics、decoherence、真实干涉律、physical Hilbert/unitary/CPTP/Kraus/density-matrix channel law、完整因果偏序、arbitrary-length causal interval、full causal set local finiteness、light cone、Lorentzian geometry、度规恢复、数据校准或经验闭合仍需要后续结构；这不削弱 S5c-S5r 与 S9-S33 的正面推进。同时，本轮结论不否定 `256 × 371` 文构造覆盖。
...
| 转移 | 下一步支持集或权重 | S11 只在 normalized amplitude support 条件下关闭 candidateWeight law，不等同 Born rule 推导 |
| 路径 | 可能历史 | 不先证明路径积分 |
...
| 方向 | 目标 | 当前状态 |
|---|---|---|
| 概率核 | 从支持集 / `Nat` 权重升级到 finite denominator interface，并补 finite row sum-one、normalized mass law 与 conditional Born-weight law | S2 已关闭有限分母接口；S9 已关闭 concrete/grid row normalization；S10 已关闭 classical finite probability law；S11 已关闭 normalized support 条件下的 Born-shaped finite probability law |
...
| 连续作用量泛函候选 | displayed continuous action-coordinate functional `S(t)=t` 的 branch sampling 接回 finite phase evolution | S24 已关闭候选接口 |
| 路径空间作用量泛函候选 | finite quotient path classes 上的 action values 接回 S5r/S24 并保持 support cancellation | S25 已关闭候选接口 |
| 有限作用量极值候选 | two-route quotient support 上 lower-minus-upper action gap 为 `1`，upper 是 finite action minimum，lower 不是 | S26 已关闭候选接口 |
| 有限因果局部性候选 | finite localFuture list 精确覆盖 one-step support，positive kernel weight respects localFuture | S27 已关闭候选接口 |
| 有限因果区间候选 | displayed two-step causal interval middle list 与 localFuture handoff | S28 已关闭候选接口 |
| 有限核路径载体候选 | finite KernelPath displayed carrier、weight readback 与 sound path-local interval | S29 已关闭候选接口 |
| 递归核路径载体候选 | finite KernelPath constructor-recursive carrier、append carrier readback 与 two-route carrier family | S30 已关闭候选接口 |
| 端点索引递归载体族候选 | endpoint-indexed recursive carrier family、carrier/weight readback 与 displayed weight sum | S31 已关闭候选接口 |
| 双路径递归载体族完备候选 | two-route displayed family upper/lower completeness 与 member weight-one law | S32 已关闭候选接口 |
| 双路径 KernelPath 枚举候选 | twoRoute source/target 任意 `KernelPath` 的 recursive carrier upper/lower normal form 与 weight-one law | S33 已关闭候选接口 |
| 逐步统一候选摘要 | 当前已关闭 finite bridge / probability / grid / action-phase / ledger boundaries 的合取摘要 | S8 已关闭当前摘要 |
| 真实干涉律 | smooth/infinite-dimensional path-space action functional、一般 path integral、continuous-time dynamics 与可测相消 | 未纳入本轮 |
| 因果局部性 | 下一步只依赖局部过去或邻域 | S27 已关闭 one-step localFuture；S28 已关闭 displayed two-step interval；S29 已关闭 finite kernel path carrier；S30 已关闭 recursive kernel path carrier；S31 已关闭 endpoint-indexed recursive carrier family；S32 已关闭 two-route displayed completeness；S33 已关闭 twoRoute source/target all-KernelPath carrier enumeration |
| 因果集接口 | 加入偏序、任意长度区间、全局局部有限性与事件网络公理 | 未纳入本轮 |
...
-> displayed continuous action functional candidate
-> smooth/infinite-dimensional path-space action functional / 一般 path integral / 真实干涉律 / 真实 channel law
-> 几何与经验接口
```
...
| 2026-05-08 | `256 × 371` correction | `machineChecked` | 新增 `QuantumRelativityWenBoundary.lean` 与《文构造完备与直相加边界》，确认文构造覆盖不受 tagged-language 边界否定 |
...
| 2026-05-08 | operator-cell grid bridge | success | 新增 `OperatorCellGridMarkovBridge.lean`，把 `371 × 256 = 94976` 覆盖网格作为 `FiniteProcess` 状态空间 |
| 2026-05-08 | stepwise plan | success | 新增 `unification-stepwise-plan.md`，记录 S0-S8 逐步验证路线；S1 已关闭 |
| 2026-05-08 | finite probability-kernel interface | success | 新增 `QuantumRelativityFiniteProbabilityBridge.lean` 与《有限概率核接口 · Markov桥S2》，关闭 concrete 与 `94976` grid 的非终端行分母非零和权重上界 |
...
| 2026-05-09 | sum-over-middle Born boundary | failure retained / success | 新增 `QuantumRelativitySumOverMiddleBornBoundaryBridge.lean` 与《sum-over-middle Born边界候选 · Markov桥S16》，关闭 composed endpoint support 到 finite Born distribution boundary 的条件式接口 |
| 2026-05-09 | unitary/CPTP ledger boundary | success | 新增 `QuantumRelativityUnitaryCPTPLedgerBridge.lean` 与《unitary-CPTP账本边界 · Markov桥S17》，关闭 current skeleton / physical required ledger boundary |
| 2026-05-09 | Born rule 推导候选 | failure retained / success | 新增 `QuantumRelativityBornRuleDerivationBridge.lean` 与《Born rule推导候选 · Markov桥S18》，关闭 Markov/amplitude compatibility 下的 finite typed-skeleton derivation |
| 2026-05-09 | 路径权重乘法候选 | failure retained / success | 新增 `QuantumRelativityPathWeightMultiplicationBridge.lean` 与《路径权重乘法候选 · Markov桥S19》，关闭 finite kernel path 的 append weight law |
| 2026-05-09 | 非平凡量子通道律候选 | failure retained / success | 新增 `QuantumRelativityNontrivialChannelLawBridge.lean` 与《非平凡量子通道律候选 · Markov桥S20》，关闭 finite support-respecting nonzero channel law |
| 2026-05-09 | 概率幅到测量权重候选 | success | 新增 `QuantumRelativityBornMeasurementBridge.lean` 与《概率幅到测量权重 · Markov桥S21》，关闭 one-qubit computational-basis normalized-state measurement weights |
| 2026-05-09 | 作用相位到测量权重链 | failure retained / success | 新增 `QuantumRelativityActionAmplitudeMeasurementBridge.lean` 与《作用相位到测量权重链 · Markov桥S22》，关闭 finite action branch 到 measurement-event weights 的推出链 |
| 2026-05-09 | 有限相位演化候选 | failure retained / success | 新增 `QuantumRelativityFinitePhaseEvolutionBridge.lean` 与《有限相位演化候选 · Markov桥S23》，关闭 finite phase evolution 到 Born-weight preservation 的边界 |
| 2026-05-09 | 连续作用量泛函候选 | failure retained / success | 新增 `QuantumRelativityContinuousActionFunctionalBridge.lean` 与《连续作用量泛函候选 · Markov桥S24》，关闭 displayed continuous action-coordinate functional 到 sampled phase evolution / Born preservation 的边界 |
| 2026-05-09 | 路径空间作用量泛函候选 | success | 新增 `QuantumRelativityPathSpaceActionFunctionalBridge.lean` 与《路径空间作用量泛函候选 · Markov桥S25》，关闭 finite quotient path space action functional 到 action phase / sampling / cancellation 的边界 |
| 2026-05-09 | 有限作用量极值候选 | failure retained / success | 新增 `QuantumRelativityFiniteActionExtremumBridge.lean` 与《有限作用量极值候选 · Markov桥S26》，修正 first build 的 redundant tactic / section close 问题后关闭 finite action gap、upper minimum 与 lower non-minimum boundary |
| 2026-05-09 | 有限因果局部性候选 | failure retained / success | 新增 `QuantumRelativityFiniteCausalLocalityBridge.lean` 与《有限因果局部性候选 · Markov桥S27》，修正 list decidability / `Fin` proof-term / singleton length 细节后关闭 finite localFuture boundary |
| 2026-05-09 | 有限因果区间候选 | failure retained / success | 新增 `QuantumRelativityFiniteCausalIntervalBridge.lean` 与《有限因果区间候选 · Markov桥S28》，修正 singleton/list membership proof-script 与 `List.mem_cons_of_mem` 参数形状后关闭 displayed two-step causal interval / localFuture handoff boundary |
| 2026-05-09 | 有限核路径载体候选 | success | 新增 `QuantumRelativityFiniteKernelPathCarrierBridge.lean` 与《有限核路径载体候选 · Markov桥S29》，关闭 displayed carrier、path weight / causal readback 与 sound two-step path-local interval |
| 2026-05-09 | 递归核路径载体候选 | failure retained / success | 新增 `QuantumRelativityKernelPathRecursiveCarrierBridge.lean` 与《递归核路径载体候选 · Markov桥S30》，修正 indexed induction 分支参数与 list equality 收尾后关闭 constructor-recursive carrier、append carrier readback 与 two-route recursive carrier family |
| 2026-05-09 | 端点索引递归载体族候选 | success | 新增 `QuantumRelativityEndpointIndexedRecursiveCarrierBridge.lean` 与《端点索引递归载体族候选 · Markov桥S31》，关闭 endpoint-indexed family member readback、carrier/weight lists 与 displayed weight sum |
| 2026-05-09 | 双路径递归载体族完备候选 | failure retained / success | 新增 `QuantumRelativityTwoRouteDisplayedCarrierCompletenessBridge.lean` 与《双路径递归载体族完备候选 · Markov桥S32》，修正 membership unfolding 与 `causalBefore` namespace 后关闭 displayed family upper/lower membership、member cases、carrier cases 与 member weight-one law |
| 2026-05-09 | 双路径 KernelPath 枚举候选 | failure retained / success | 新增 `QuantumRelativityTwoRouteKernelPathEnumerationBridge.lean` 与《双路径KernelPath枚举候选 · Markov桥S33》，修正 indexed-inductive recursion / fixed-index cases 证明方式后关闭 source/target all-KernelPath carrier normal form、weight-one law 与 displayed-family readback |
...
> Markov 因果桥目前把“测量结果成为事件”压成有限、双投影、可验证的 skeleton；本轮已关闭抽象桥、三状态 concrete witness、`94976` operator-cell grid bridge、S2 有限概率核分母接口、S3 路径/因果约束、S4 量子振幅-通道候选分层、S5 干涉 / Born-shaped 候选接口、S5b 非零路径振幅候选 witness、S5c 双路径有限相消候选、S5d 离散相位标记候选、S5e 离散作用量相位候选、S5f 有限路径族求和候选、S5g 有限路径族求和代数候选、S5h 端点索引路径族候选、S5i 端点支撑规范化候选、S5j 双路径枚举候选、S5k 路径身份键候选、S5l 有限键商候选、S5m 路径商类候选、S5n 规范代表元候选、S5o 商支撑枚举候选、S5p 商支撑代数候选、S5q 观测账本候选、S5r 作用量相位律候选、S8 逐步统一候选摘要、S9 有限概率归一化候选、S10 归一化质量求和候选、S11 Born 权重条件归一候选、S12 Born 分布边界候选、S13 channelCompose 候选、S14 channelCompose 结合律候选、S15 sum-over-middle 通道组合候选、S16 sum-over-middle Born 边界候选、S17 unitary/CPTP 账本边界、S18 Born rule 推导候选、S19 路径权重乘法候选、S20 非平凡量子通道律候选、S21 概率幅到测量权重候选、S22 作用相位到测量权重链、S23 有限相位演化候选、S24 连续作用量泛函候选、S25 路径空间作用量泛函候选、S26 有限作用量极值候选、S27 有限因果局部性候选、S28 有限因果区间候选、S29 有限核路径载体候选、S30 递归核路径载体候选、S31 端点索引递归载体族候选、S32 双路径递归载体族完备候选与 S33 双路径 KernelPath 枚举候选，后续处理 stochastic semantics、general all-path enumeration、smooth/infinite-dimensional path-space action functional、continuous variation space、stationary action principle、general Hamiltonian/unitary amplitude dynamics、general Hilbert measurement、POVM/PVM、decoherence、一般 path integral、physical unitary/CPTP/Kraus/density-matrix channel law、完整因果集、arbitrary-length causal interval、full causal set local finiteness、light cone、Lorentzian geometry、几何极限与带数据校准的经验接口。
