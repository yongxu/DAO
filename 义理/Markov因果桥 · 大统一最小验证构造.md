...
| Born rule 从 Markov / 振幅桥的推导 | 未纳入本轮 | S12 不证明无条件振幅平方律推导或物理测量概率律 |
...
| 轴 | 状态 | 说明 |
|---|---|---|
| Born rule 从 Markov 桥的推导 | 未纳入本轮 | S12 已关闭 finite Born distribution boundary；仍未从振幅结构推出无条件物理测量概率律 |
...
尚未闭合的范围同样应实事求是列出：channel composition、Born rule 从 Markov / 振幅桥的推导、真实干涉律、真实 quantum channel law、完整因果偏序、度规恢复、数据校准或经验闭合仍需要后续结构；这不削弱 S5c-S5r、S9、S10、S11 与 S12 的正面推进，只防止把候选边界误读成已完成的物理定律。同时，本轮结论不否定 `192 × 371` 文构造覆盖。
