...
本轮闭合范围：**Born rule 从 Markov row probability 与 amplitude bridge compatibility 的有限 typed-skeleton 推导**。S21 已把 one-qubit computational-basis 版本接到显式 measurement weights：`bornProb0` / `bornProb1` 非负且和为 `1`；S22 进一步把 S5r 的 finite action branch 接到该 measurement-weight 边界。
